...
class CurrentUser:
    ...
    def __init__(self, user_id: uuid.UUID, restaurant_id: uuid.UUID | None, role: RoleEnum):
...
async def get_current_user(
    ...
    Extracts user_id, restaurant_id (optional for SUPERADMIN), and role from claims.
    """
...
def require_role(*allowed_roles: RoleEnum):
    ...
    allowed roles. Usage: Depends(require_role(RoleEnum.RESTAURANT_ADMIN))
    """
...
RequireAdmin = Annotated[
    CurrentUser,
    Depends(require_role(RoleEnum.SUPERADMIN, RoleEnum.RESTAURANT_ADMIN, RoleEnum.MANAGER)),
]
...
def tenant_scoped_query(
    ...
    """
    Append .where(model.restaurant_id == restaurant_id) to any query.
    If current_user is a SUPERADMIN and no specific restaurant_id is provided, bypass tenant scoping!
    """
    if current_user and current_user.role == RoleEnum.SUPERADMIN and restaurant_id is None:
        return stmt
    if restaurant_id is not None:
        return stmt.where(model.restaurant_id == restaurant_id)
    return stmt
