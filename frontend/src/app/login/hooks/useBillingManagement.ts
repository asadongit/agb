...
export function useBillingManagement({
  ...
  const { computedStartDate, computedEndDate } = useMemo(() => {
    ...
    if (dateRangeMode === "today") {
      ...
      return { computedStartDate: customStartDate, computedEndDate: customEndDate };
    }
  ...
  const { dailyGrandTotal, dailyNetPaid } = useMemo(() => {
    let grandTotal = 0;
    let netPaid = 0;
    billsList.forEach((b) => {
      const s = (b.status || "").toUpperCase();
      if (s === "PAID" || s === "SERVED" || s === "COMPLETED" || s === "FINALIZED") {
    ...
    return { dailyGrandTotal: grandTotal, dailyNetPaid: netPaid };
  }, [billsList]);
  ...
  const [editingCompletedBill, setEditingCompletedBill] = useState<ManualBill | null>(null);
  ...
  const loadBillingData = useCallback(async () => {
    ...
    try {
      const [billsRes, approvalsRes, countRes] = await Promise.all([
        apiRequest<ManualBill[]>(`/api/billing/bills?start_date=${computedStartDate}&end_date=${computedEndDate}`).catch(() => []),
  ...
  }, [apiRequest, authHeaders, computedStartDate, computedEndDate]);
  ...
  const handleCreateBill = async (proceedToPayment = false) => {
    ...
    try {
      const payload = {
        ...
        ...(editingCompletedBill ? { replaces_bill_id: editingCompletedBill.id } : {})
      };
    ...
    if (bill.items && bill.items.length > 0) {
      const itemsMapped: DraftCartItem[] = bill.items.map((it: any) => {
        ...
        return {
          ...
          selected_unit: it.selected_unit || it.unit || null,
          base_unit_price: uPrice,
          base_mrp: uMrp,
        };
      });
  ...
  const handleEditCompletedBill = (bill: ManualBill) => {
    setEditingCompletedBill(bill);
    setSelectedTable(bill.basket_number || "WALK-IN");
    setCustomerName(bill.customer_name || "");
    setCustomerPhone(bill.customer_phone || "");
    setIsInterstate(Boolean((bill as any).is_interstate));
  ...
  const handleApplyDiscount = async (itemComplimentaryQuantities?: Record<string, number>, isItemLevelComp?: boolean) => {
    ...
    try {
      const res = await apiRequest<ManualBill>(`/api/billing/bills/${discountTargetBill.id}/apply-discount`, {
        method: "POST",
        body: JSON.stringify({
          discount_type: isItemLevelComp ? "COMPLIMENTARY_ITEMS" : discountType,
          discount_value: discountValue,
          reason_note: discountReason || "Complimentary items",
          item_complimentary_quantities: itemComplimentaryQuantities || null,
        }),
      });
    ...
    try {
      await apiRequest<ManualBill>(`/api/billing/bills/${paymentTargetBill.id}/mark-paid`, {
        ...
        body: JSON.stringify({
          ...
          change_denominations: changeDenominations || null,
  ...
  return {
    ...
    setEditingCompletedBill,
    handleEditCompletedBill,
    handleKeepAsDraft,
