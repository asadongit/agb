      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-y-auto p-4 sm:p-6 lg:p-8">
        ...
        <div className="mb-6 flex items-center justify-between pb-4 border-b border-[var(--border-subtle)]">
          ...
          <div className="flex items-center gap-3">
            ...
            {activeTab === "billing" && (
              <div className="hidden sm:flex items-center gap-2 mr-2 bg-[var(--bg-surface-elevated)] border border-[var(--border-strong)] rounded-xl px-3 py-1.5 shadow-xs">
                <select
                  value={billingState.dateRangeMode}
                  onChange={(e) => billingState.setDateRangeMode(e.target.value as any)}
                ...
                {billingState.dateRangeMode === "custom" && (
                  <div className="flex items-center gap-1">
                    <input 
                      type="date"
                      value={billingState.customStartDate}
                      onChange={(e) => billingState.setCustomStartDate(e.target.value)}
                      className="bg-[var(--bg-surface)] text-[10px] font-bold text-[var(--text-primary)] focus:outline-none cursor-pointer rounded px-1"
                    />
                    <span className="text-[10px] text-[var(--text-muted)]">-</span>
                    <input 
                      type="date"
                      value={billingState.customEndDate}
                      onChange={(e) => billingState.setCustomEndDate(e.target.value)}
                      className="bg-[var(--bg-surface)] text-[10px] font-bold text-[var(--text-primary)] focus:outline-none cursor-pointer rounded px-1"
                    />
                  </div>
                )}
                ...
                <div className="flex items-center gap-1.5 text-xl font-black">
                  <span className="text-sm font-semibold text-[var(--text-muted)] mt-1">Total:</span>
                  <span className="font-mono text-[var(--text-primary)]">₹{billingState.dailyGrandTotal.toFixed(2)}</span>
                </div>
                <div className="h-4 w-px bg-[var(--border-strong)] mx-1"></div>
                <div className="flex items-center gap-1.5 text-xl font-black">
                  <span className="text-sm font-semibold text-[var(--text-muted)] mt-1">Net:</span>
