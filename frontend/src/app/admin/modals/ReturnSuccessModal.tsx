...
export function ReturnSuccessModal({
  ...
  const handleView = () => {
    generateReturnReceiptPDF(returnData, restaurantName, "view");
  };

  const handleDownload = () => {
    generateReturnReceiptPDF(returnData, restaurantName, "download");
  };
              ...
              {(returnData.returned_items || []).map((item, idx) => (
                <div key={idx} className="flex justify-between items-center text-xs font-mono text-[var(--text-primary)]">
                  ...
                  <span className="font-bold text-sky-400">
                    ₹{(item.line_refund !== undefined ? item.line_refund : item.quantity * item.unit_price).toFixed(2)}
                  </span>
                </div>
              ))}
            </div>
